...
function initMeScroll(mescrollId, options) {
	...
	var myOption={
		down:{
			offset: 85, //触发刷新的距离
			htmlContent: '<p class="downwarp-progress downwarp-progress-slow"></p>', //布局内容
...
//			},
			inOffset: function(mescroll) {
				//进入指定距离范围内那一刻的回调
				mescroll.downProgressDom.classList.remove("downwarp-progress-fast");
				mescroll.downProgressDom.classList.add("downwarp-progress-slow");
			},
			outOffset: function(mescroll) {
				//下拉超过指定距离那一刻的回调
				mescroll.downProgressDom.classList.remove("downwarp-progress-slow");
				mescroll.downProgressDom.classList.add("downwarp-progress-fast");
			},
			...
			showLoading: function(mescroll) {
				//触发下拉刷新的回调
				//配置空方法,表示showLoading不做处理 (不可写showLoading:null,否则会执行mescroll默认配置的showLoading方法)
			}
		},
		up:{
			toTop: {
