...
function initMeScroll(mescrollId, options) {
	...
	var myOption={
		down:{
			htmlContent: '<p class="downwarp-progress downwarp-loading"></p>', //布局内容
...
//			},
			inOffset: function(mescroll) {
				//进入指定距离范围内那一刻的回调
				mescroll.downProgressDom.classList.remove("downwarp-loading");
				mescroll.downProgressDom.classList.add("downwarp-progress");
			},
			outOffset: function(mescroll) {
				//下拉超过指定距离那一刻的回调
				mescroll.downProgressDom.style.height="50px";
				mescroll.downProgressDom.classList.add("downwarp-loading");
				mescroll.downProgressDom.classList.remove("downwarp-progress");
			},
			onMoving: function(mescroll, rate, downHight) {
				//下拉过程中的回调,滑动过程一直在执行; rate下拉区域当前高度与指定距离的比值(inOffset: rate<1; outOffset: rate>=1); downHight当前下拉区域的高度
				if(rate<1) mescroll.downProgressDom.style.height=downHight/2+"px";
			},
			showLoading: function(mescroll) {
				//触发下拉刷新的回调; 此处无逻辑,但不可写成showLoading:null,否则会使用mescroll默认配置的逻辑
			}
		},
		up:{
			toTop: {
